...
def get_program_path():
    """获取程序自身路径，兼容 Python 源码和 PyInstaller 打包程序"""
    if getattr(sys, "frozen", False):
        return Path(sys.executable).resolve()
    return Path(__file__).resolve()


# 在切换工作目录前固定程序路径，避免相对路径受 os.chdir() 影响
...
def find_latest_output_file(filename):
    """查找同类型最新的时间命名结果，并兼容旧的固定文件名"""
    file_path = Path(filename)
    candidates = list(file_path.parent.glob(
        f"{file_path.stem}_*{file_path.suffix}"
    ))
...
def select_csv_file():
    """选择CSV文件"""
    default_csv = find_latest_output_file("result.csv") or "result.csv"
    ...
    while True:
        ...
        if os.path.exists(csv_file):
            ...
            print("请确保文件路径正确，或先运行测速生成带时间的结果文件")
...
def handle_proxy_mode():
    ...
    # 生成反代IP列表
    print(f"\n正在处理CSV文件: {csv_file}")
    proxy_file = create_timestamped_filename("ips_ports.txt")
    success = generate_proxy_list(csv_file, proxy_file)
    ...
    if success:
        ...
        # 运行测速
        result_code, result_file = run_speedtest_with_file(
            proxy_file, dn_count, speed_limit, time_limit, thread_count
        )
        
        # 如果测速成功，询问是否上报结果
        if result_code == 0 and result_file and os.path.exists(result_file):
            upload_results_to_api(result_file)
...
def handle_beginner_mode(ip_file=CLOUDFLARE_IP_FILE, ip_version="ipv4"):
    ...
    result_file = create_timestamped_filename("result.csv")
...
def generate_proxy_list(result_file="result.csv", output_file=None):
    """从测速结果生成反代IP列表"""
    if output_file is None:
        output_file = create_timestamped_filename("ips_ports.txt")
...
def run_speedtest_with_file(ip_file, dn_count, speed_limit, time_limit, thread_count="200"):
    ...
    try:
        ...
        # 等待用户按键，不自动关闭窗口
        input("\n按回车键退出...")
        return result.returncode, result_file if result.returncode == 0 else None
...
示例:
  ...
  python cloudflare_speedtest.py --mode beginner --upload github --repo owner/repo --token ghp_xxx
    ...
    parser.add_argument('--csv', type=str, default=None,
                       help='CSV文件路径（优选反代模式，默认使用最新的 result_时间.csv）')
...
def run_with_args(args):
    ...
    if args.mode == 'beginner':
        ...
        if result.returncode == 0:
            ...
            if args.upload == 'api':
                if not args.worker_domain or not args.uuid:
                    ...
                    # 调用命令行模式的上传函数
                    upload_to_cloudflare_api_cli(result_file, args.worker_domain, args.uuid, args.upload_count, clear_existing=args.clear)
            elif args.upload == 'github':
                if not args.repo or not args.token:
                    print("❌ GitHub上传需要提供 --repo 和 --token 参数")
                else:
                    # 调用命令行模式的上传函数
                    upload_to_github_cli(result_file, args.repo, args.token, args.file_path, args.upload_count)
        else:
            print("\n❌ 测速失败")
            return 1
        ...
        # 优选反代模式
        csv_file = args.csv or find_latest_output_file("result.csv")

        print(f"\n[优选反代模式]")
        print(f"  CSV文件: {csv_file or '未找到'}")
        
        if not csv_file or not os.path.exists(csv_file):
            print("❌ 未找到可用的测速结果CSV文件")
            return 1
...
def main():
    """主函数"""
    # 无论是双击、命令行还是定时任务启动，都在程序所在目录读写文件
    set_program_working_directory()
